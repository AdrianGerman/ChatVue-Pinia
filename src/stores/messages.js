...
export default defineStore("messages", {
  state: () => ({
    messages: [
      ...
      },
      {
        id: 11,
        author: 1,
        message: "Pues, ¡acabamos de lanzar el nuevo motor de lector de codigo de Vue.js!",
        timestamp: new Date().toLocaleTimeString(),
        channelId: 6,
        read: false,
      },
  ...
  getters: {
    ...
    countUnreadMessagesByChannelId: (state) => (channelId) => {
      return state.findMessagesByChannelId(channelId).filter((message) => message.read === false)
        .length;
    },
  },
  actions: {
